  ...
  .query(async ({ ctx, input }) => {
    ...
    const [
      ...
      ctx.db
        ...
        .where(eq(gadhawaAgriculturalLand.familyId, input.id)),
      ctx.db
        .select()
        .from(gadhawaAnimal)
        .where(eq(gadhawaAnimal.familyId, input.id)),
      ctx.db
        .select()
        .from(gadhawaAnimalProduct)
        .where(eq(gadhawaAnimalProduct.familyId, input.id)),
      ctx.db
        .select()
        .from(gadhawaCrop)
        .where(eq(gadhawaCrop.familyId, input.id)),
      ctx.db
        .select()
        .from(gadhawaIndividual)
        .where(eq(gadhawaIndividual.familyId, input.id)),
    ]);
