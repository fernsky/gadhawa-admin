...
export const getAll = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    if (filters) {
      ...
      if (filters.wardNo) {
        filterConditions.push(eq(gadhawaIndividual.wardNo, filters.wardNo));
      }
      if (filters.familyId) {
        filterConditions.push(eq(gadhawaIndividual.familyId, filters.familyId));
      }
      if (filters.gender) {
        filterConditions.push(eq(gadhawaIndividual.gender, filters.gender));
      }
      if (filters.ageRange) {
        if (filters.ageRange.min !== undefined) {
          filterConditions.push(sql`${gadhawaIndividual.age} >= ${filters.ageRange.min}`);
        }
        if (filters.ageRange.max !== undefined) {
          filterConditions.push(sql`${gadhawaIndividual.age} <= ${filters.ageRange.max}`);
        }
      }
...
export const getById = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    const individual = await ctx.db
      .select()
      .from(gadhawaIndividual)
      .where(eq(gadhawaIndividual.id, input.id))
      .limit(1);
...
export const getStats = publicProcedure.query(async ({ ctx }) => {
  const stats = await ctx.db
    .select({
      ...
      averageAge: sql<number>`ROUND(AVG(${gadhawaIndividual.age}))::integer`}).from(gadhawaIndividual);
