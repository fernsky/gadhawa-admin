...
import { gadhawaDeath } from "@/server/db/schema/family/deaths";
...
export const getAll = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    ...
    if (filters) {
      ...
      if (filters.wardNo) {
        filterConditions.push(eq(gadhawaDeath.wardNo, filters.wardNo));
      }
      if (filters.deceasedName) {
        filterConditions.push(
          ilike(gadhawaDeath.deceasedName, `%${filters.deceasedName}%`),
        );
      }
      if (filters.deceasedGender) {
        filterConditions.push(
          eq(gadhawaDeath.deceasedGender, filters.deceasedGender),
        );
      }
...
export const getById = publicProcedure
  ...
  .query(async ({ ctx, input }) => {
    const death = await ctx.db
      .select()
      .from(gadhawaDeath)
      .where(eq(gadhawaDeath.id, input.id))
      .limit(1);
...
export const getStats = publicProcedure.query(async ({ ctx }) => {
  const stats = await ctx.db
    .select({
      ...
      avgAge: sql<number>`avg(${gadhawaDeath.deceasedAge})`,
    })
    .from(gadhawaDeath);
